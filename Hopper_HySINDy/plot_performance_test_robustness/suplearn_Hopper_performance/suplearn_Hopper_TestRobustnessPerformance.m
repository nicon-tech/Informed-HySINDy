%% Hopping performance (number of points in a cluster k) - supervised learning
% Run Hopping Model
...
% thick_unddata
% model stiffness
thick_unddata_vect = 0.01:0.01:0.24;
eps_vect = [1e-6 1e-4 1e-2];
k_vect = 6:10; %vector made by the values k used in knn
numleveldataprocessed = 3; %number of level data processed
% tStart = zeros(numel(thick_unddata_vect),numel(k_vect));
tEnd = zeros(numel(thick_unddata_vect),numel(k_vect),numleveldataprocessed,numel(eps_vect));
a0_mean = zeros(numel(thick_unddata_vect),numel(k_vect),numleveldataprocessed,numel(eps_vect));
a0_median = zeros(numel(thick_unddata_vect),numel(k_vect),numleveldataprocessed,numel(eps_vect));
% a0_trimmean = zeros(numel(thick_unddata_vect),numel(k_vect),numleveldataprocessed,numel(eps_vect));
%%
for leveldataprocessed=1:3 %level of data processed
    ...
    for eps = eps_vect
        ...
        for thick_unddata = thick_unddata_vect
            %% data generation
            ...
            %% undefined data
            ind_knn = 1;
            for k=k_vect % number of points in a cluster %k = dimMan+2;
                tStart = tic;
...
%                 x2 = y_out_und(1:end-1,1); % height
                data2 = y_out_und(1:end-1,2); % velocity
                
                numOfPts = length(data2); % number of points in the data
...
%                         disp(['Model num:' num2str(kk) ' of ' num2str(length(Xicomb)) ' for this cluster'])
                ...
                a0_mean(ind_thick_unddata,ind_knn,leveldataprocessed,ind_eps) = xswitch_mean; %switch position calculate with mean
                a0_median(ind_thick_unddata,ind_knn,leveldataprocessed,ind_eps) = xswitch_median; %switch position calculate with median
                
                switch_index_spring = find(y_out_und(:,1)<=xswitch_median);
                new_data_t_spring = t_out_und(switch_index_spring,:);
                ...
                tEnd(ind_thick_unddata,ind_knn,leveldataprocessed,ind_eps) = toc(tStart);
                ind_knn = ind_knn+1;
            end
 ...
 save([datestr(now, dateformatout)...
     'Hopping_data_TestRobustnessPerformance.mat'])
...
savefig(plotname)
%% plot switch position vs thick_undefined_data & k_knn
%
plotname = [pathname datestr(now, dateformatout) 'xswitchMean_vs_PercUndData.fig'];
figure(2345)

for ii=1:3 %run for each noise level
    for jj=1:numleveldataprocessed %plot switch position for each level data processed
        subplot(3,3,jj+3*(ii-1))
        for kk=1:numel(k_vect)
            PercUndData = 100.*thick_unddata_vect./(max(y_out_flight(:,1))-min(y_out_spring(:,1)));
%             plot(PercUndData,(abs(a0_mean(:,kk,jj,ii)-1)));%type plot1
            plot(PercUndData,(abs(a0_mean(:,kk,jj,ii)-1))'./thick_unddata_vect);%type plot2
            hold on
            xlimits = [min(PercUndData) max(PercUndData)];
...
%% plot switch position vs thick_undefined_data & k_knn
%
plotname = [pathname datestr(now, dateformatout) 'xswitchMedian_vs_PercUndData.fig'];
figure(2346)
...
for ii=1:3 %run for each noise level
    for jj=1:numleveldataprocessed %plot switch position for each level data processed
        subplot(3,3,jj+3*(ii-1))
        for kk=1:numel(k_vect)
            PercUndData = thick_unddata_vect./(max(y_out_flight(:,1))-min(y_out_spring(:,1)));
%             plot(PercUndData,abs(a0_median(:,kk,jj,ii)-1));%type plot1
            plot(PercUndData,(abs(a0_median(:,kk,jj,ii)-1))'./thick_unddata_vect);%type plot2
            hold on
            xlimits = [min(PercUndData) max(PercUndData)];
%             ylimits = [0.9*min(min(min(min(abs(a0_median-1))))) 1.02*max(max(max(max(abs(a0_median-1)))))];
            ylimits = [0 1];
            axis([xlimits ylimits])
            l = get(gca, 'Children');
            xlabel('% Und. Data [ ]','Interpreter','latex')
            ylabel('$\epsilon_{xswitch-median}$ [ ]','Interpreter','latex')
            legend('k = 6','k = 7','k = 8','k = 9','k = 10')
            txt = ['i.c. ' int2str(jj) ' eps1e- ' int2str(6-2*(ii-1))];
            title(txt)
%             subtitle(['error' int2str(ii)])
...
savefig(plotname)
%%
% [output] = RUN_Hopper_approx(p,Xicomb_spring,Xicomb_flight,xswitch_max);