...
delta_aic_c = Cluster_val_spring.aic_c-min(Cluster_val_spring.aic_c)
nLib = length(Cluster_spring.Xistruct.Xicomb);
figure(1)
ii =1;
dateformatout = 'mmddyyyy';
plotname = [pathname datestr(now, dateformatout) 'Coeff 1 .fig']
for  kk = 1:nLib
    
    vc = Cluster_spring.Xistruct.Xicomb{kk}(:,1)
    ac = Cluster_spring.Xistruct.Xicomb{kk}(:,2)
...
figure(119)
plot(y_val_spring(:,1), y_val_spring(:,2),'o')
hold on
...
jj = 1;
valsims_spring = Cluster_val_spring.valsims;
Xicomb = Cluster_spring.Xistruct.Xicomb;
for kk = 1 % correct model for Cluster 1
    
    Xi = Xicomb{kk}
    clear  savetB savexB
    savetB = valsims_val_spring{kk}.savetB;
    savexB = valsims_val_spring{kk}.savexB;
    
    idx_xy2 = knnsearch([x2 y2],...
        [min(x2) 0],'K',k_val_spring);
    idx_xy2 = idx_xy2(:,2:end);
    
    val_ntimes = floor(0.4/dt); %length of comparision not including ICs
    data = [x2 y2];
    [xA, x0clust, tvectest] = buildinit_fromcluster(val_ntimes, 1, idx_xy2, data, dt, k_val_spring);
...
%% relative AIC_c plot
close all
modelnum = 2; %  this is the modelnumber in the HighFreq_II library
