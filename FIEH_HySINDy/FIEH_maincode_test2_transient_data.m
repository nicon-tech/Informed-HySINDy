%% FIEH - supervised learning - Test for transient data
...
thick_unddata = 0.01;%0.013; % range undefined data position
...
tend = 11;%17; %TEST NICO 4.1373 time to complete an orbit; 12 time for training; 17 time for predict
...
for ii=1:size(yinit,1)
    for jj=1:size(Yi,1)
        ...
        clear ind_stab
        t_out_ode45 = t_out_ode45(t_out_ode45>=0);
...
tend = 9.17;%17; %TEST NICO 4.1373 time to complete an orbit; 12 time for training; 17 time for predict
dt = 0.001;%0.0001; %TEST NICO 0.0001
tspan = 0:dt:tend;
...
tspan_impact = 0;
tspan_free = 0;
% for mm=1:numel(t_out_impactDown)-144 %we cut 144 points to avoid that springing dynamic continue beyond its own manifold
%     tspan_impact = [tspan_impact mm*dt];
% end
% for mm=1:numel(t_out_free)-268%we cut 268 points to avoid that flying dynamic continue beyond its own manifold
%     tspan_free = [tspan_free mm*dt];
% end
Xicomb = Cluster_impactDown.Xistruct.Xicomb;
for kk = 4%:length(Xicomb)
...
for kk = 7%:length(Xicomb)
    disp('Cluster top impact')
    disp(['Model num:' num2str(kk) ' of ' num2str(length(Xicomb)) ' for this cluster'])
    Xi_impact_top = [Xicomb{kk}(:,1), Xicomb{kk}(:,2)]
    [t_mod_impactTop,x_mod_impactTop] = ode45(@(t,x)sparseGalerkin(t,x,Xi_impact_top,polyorder_impactTop,polyorder_impactTop_x,polyorder_impactTop_y,usesine_impactTop),[0 50],y0_impactTop,options);  % approximate
