...
for kk = 1:ninits % for each initial condition
    ...
    sol=ode45(@(t,x)sparseGalerkin(t,x,Xi,polyorder,polyorderx,polyordery,usesine),...
    ...
    ind_tlast = find(tvec<=(max(sol.x)), 1, 'last'); %Look for the last n nonzero elements in X using find(X,n,'last')
