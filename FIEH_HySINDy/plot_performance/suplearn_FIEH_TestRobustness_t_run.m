%% t_run FIEH performance - supervised learning
...
plottag =1;
addpath('FIEH'); addpath('SINDy'); addpath('validation'); addpath('utils_validation');
...
thick_unddata_vect = 0.00025:0.0005:0.0248;
eps_vect = [1e-8 1e-6 1e-4];
numleveldataprocessed = 1; %number of level data processed - not analysed
% tStart = zeros(1,size(thick_unddata_vect,2));
tEnd = zeros(numel(thick_unddata_vect),numel(eps_vect));
zswitch_median_down = zeros(numel(thick_unddata_vect),numel(eps_vect));
zswitch_median_top = zeros(numel(thick_unddata_vect),numel(eps_vect));
...
for eps = eps_vect %noise level
    ...
    for thick_unddata = thick_unddata_vect %range undefined data position
        ...
        %initial condition
        yinit = [0.01 0.02 2*pi*6]; %for training
%         yinit = [0.02 0.025 2*pi*6]; %for validation
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [t_out,y_out] = ode45(@(t,y) fieh(t,y,Ca,C,M,Yi,omega,kappa,stroke,mu,g,N,D,mu0,m,R,L),tspan,yinit);
        
        % a = fieh(t_out,y_out',Ca,C,M,Yi,omega*ones(1,numel(y_out(:,3))),kappa,stroke,mu,g,N,D,mu0,m,R,L)';
        
        %
        z_out = [(y_out(:,1)-Yi*sin(y_out(:,3))) , (y_out(:,2)-Yi*omega*cos(y_out(:,3)))]; %augmented data state
        ...
        t_out = t_out(t_out>=4); %t_out>=3.9708 in the old version
...
% % %         title('phase space data')
        ...
        %similar dynamic data => each cluster is related to its own library
        idx_xy = idx_xy(:,2:end);
        ...
        for ii = 1:floor(numOfPts)
            ...
            polyorder_und = 1;  % search space up to i-th order polynomials %trial 2
            polyorder_und_x = 1;  % search space up to i-th order polynomials %trial 1
            polyorder_und_y = 0;  % search space up to i-th order polynomials %trail 1
        ...
        for ii = 1:length(Cluster)
            ...
            for kk = 1:length(Xicomb)
                ...
                for ll = 1:length(xA)
                    clear z_switch nodynam1 abserror abserror_avg1 RMSE xAcomp xBcomp
        ...
        [idxkmeans,Ckmeans] = kmeans(Cluster_zswitch_max,13,'Distance','cityblock',...
            'Replicates',13,'Options',optskmeans);
% % %         figure; %clustering of possible switches
...
% % %         
        ...
        zswitch_median_down(ind_thick_unddata,ind_eps) = z_out_und(idx_pos1,1); %switch position 1
        %
        ind_positive = find(z_out_und(:,1)>0);
        [~,idxtop]=min(abs(z_out_und(ind_positive,2)-zswitch_median(:,2)));
        idx_pos2 = ind_positive(idxtop);
        zswitch_median_top(ind_thick_unddata,ind_eps) = z_out_und(idx_pos2,1); %switch position 2
        % zswitch_median_down = zswitch_median(1,1); %switch position 1
        % zswitch_median_top = zswitch_median(1,2); %switch position 2
        
        % true switches
        zswitch_median_down_true = -0.015; %switch position 1
        zswitch_median_top_true = 0.015; %switch position 2
        
        % We find switch position by use xswitch_max because the algorithm
        % findchangepts find the switch prematurely. Then we use mean to add
        % statistic
        
        switch_index_impactDown = find(z_out(:,1)<=zswitch_median_down_true);
        ...
        %
        switch_index_impactTop = find(z_out(:,1)>=zswitch_median_top_true);
        ...
        %
        switch_index_free = find(z_out(:,1)>zswitch_median_down_true & z_out(:,1)<zswitch_median_top_true);
        ...
        %%
        tEnd(ind_thick_unddata,ind_eps) = toc(tStart); % pair 1: toc
...
% save([datestr(now, dateformatout) 'FIEH_data_eps.mat'])
...
%% plot trun vs thick_undefined_data and noise
%
% input a save location in the line below:
pathname = 'plot_suplearn_FIEH_trun';
% mkdir(pathname);

plotname = [pathname datestr(now, dateformatout) 'trun_vs_thickunddata.fig'];
figure(1234)
for kk=1:3 %plot trun for each level noise processed
    %     plot(thick_unddata_vect,nonzeros(tEnd(:,kk))','LineWidth',1.5);
    PercUndData = 2.*100.*thick_unddata_vect./(max(z_out(:,1))-min(z_out(:,1)));
    plot(PercUndData,(tEnd(:,kk))','LineWidth',1.5);
    hold on
    xlimits = [min(PercUndData) max(PercUndData)];
    ylimits = [0.9*min(min(tEnd)) 1.1*max(max(tEnd))];
    axis([xlimits ylimits])
    l = get(gca, 'Children');
    xlabel('% Und. Data [ ]','Interpreter','latex')
    ylabel('$t_{run}$ [s]','Interpreter','latex')
    %     axis tight
    legend('num-i.c. eps1e-8 = 3','num-i.c. eps1e-6 = 3','num-i.c. eps1e-4 = 3')
    % set(gca,'FontSize',13)
...
figure(2345)
for kk=1:3 %plot trun for each level noise processed
    plot(thick_unddata_vect,nonzeros(zswitch_median_down(:,kk))','LineWidth',1.5);
    PercUndData = 2.*100.*thick_unddata_vect./(max(z_out(:,1))-min(z_out(:,1)));
    %     plot(PercUndData,nonzeros(a0_median(:,kk))','LineWidth',1.5);
    plot(PercUndData,(zswitch_median_down(:,kk)));
    plot(PercUndData,(zswitch_median_down(:,kk)));
    hold on
    xlimits = [min(PercUndData) max(PercUndData)];
    ...
    l = get(gca, 'Children');
    xlabel('% Und. Data [ ]','Interpreter','latex')
    ylabel('$switch-position$ [ ]','Interpreter','latex')
    %     axis tight
    legend('num-i.c. eps1e-8 = 3','num-i.c. eps1e-6 = 3','num-i.c. eps1e-4 = 3')
    % set(gca,'FontSize',13)
    title('switch-position  vs  thick_{undefined-data}')
    set(gca,'FontSize',9)
    set(l, 'linewidth', 1.5)
end
